...
export default function PolacoTrabajo() {
  ...
  return (
    <main className="min-h-screen bg-cream font-sans">
      ...
      <div className="max-w-4xl mx-auto px-8 py-10">
        <div className="mb-8">
          <div className="text-xs font-bold text-teal tracking-widest uppercase mb-2">Curso especializado</div>
          <h1 className="font-display text-3xl font-extrabold text-navy tracking-tight mb-2">Polaco para el Trabajo</h1>
          <p className="text-navy/50 text-sm max-w-xl">Vocabulario esencial para trabajar en plantas de produccion de alimentos en Polonia: turnos, seguridad, comandos de linea y herramientas.</p>
        </div>

        <div className="bg-navy rounded-2xl p-6 mb-8">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm text-white/60">Progreso del curso</span>
            <span className="text-2xl font-extrabold text-teal">{porcentajeTotal}%</span>
          </div>
          <div className="h-2 bg-white/10 rounded-full overflow-hidden mb-3">
            <div className="h-full bg-teal rounded-full transition-all" style={{ width: `${porcentajeTotal}%` }}></div>
          </div>
          <div className="flex justify-between text-xs text-white/30">
            <span>{totalCompletadas} de {totalLecciones} lecciones</span>
            {leccionSiguiente && (
              <Link href={`/idioma/curso/${leccionSiguiente}`} className="text-teal font-semibold hover:underline">Continuar →</Link>
            )}
          </div>
        </div>
          ...
          {modulos.map((m) => (
            <div key={m.id} className="bg-white rounded-2xl border border-navy/8 overflow-hidden">
              <div className="px-5 py-4 flex items-center justify-between border-b border-navy/5">
                <div>
                  <div className="text-xs font-bold text-teal uppercase tracking-widest mb-1">Modulo {m.orden}</div>
