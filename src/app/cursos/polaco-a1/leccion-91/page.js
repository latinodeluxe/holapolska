...
export default function Leccion91() {
  const [tab, setTab] = useState('repaso')
  const [respuestas, setRespuestas] = useState({})
  const [mostrarResultado, setMostrarResultado] = useState(false)
  const repaso = [
    { categoria: 'Saludos', frases: [
      { polaco: 'Czesc / Dzien dobry', pronunciacion: 'cheshch / dyen DOB-ri', traduccion: 'Hola / Buenos dias' },
      { polaco: 'Nazywam sie / Jestem z', pronunciacion: 'na-ZI-vam she / YES-tem z', traduccion: 'Me llamo / Soy de' },
    ]},
    { categoria: 'Verbos clave', frases: [
      { polaco: 'jestem / jestes / jest', pronunciacion: 'YES-tem / YES-tesh / yest', traduccion: 'soy / eres / es' },
      { polaco: 'ja / ty / on / ona', pronunciacion: 'ya / ti / on / O-na', traduccion: 'yo / tu / el / ella' },
    ]},
  ]
  const examen = [
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        <div className="mb-8">
          <span className="text-sm text-red-500 font-medium">Leccion 91 · Modulo 10</span>
          <h1 className="text-3xl font-bold text-gray-900 mt-1">Repaso saludos y presentaciones</h1>
          <p className="text-gray-500 mt-2">Repasa el Modulo 1</p>
        </div>
          ...
          <div>
            {!mostrarResultado ? (
              ...
              <div className="bg-white rounded-2xl p-8 shadow-sm text-center">
                ...
                <a href="/cursos/polaco-a1/leccion-92" className="inline-block mt-6 px-6 py-3 bg-red-500 text-white rounded-xl font-semibold">Siguiente leccion</a>
              </div>
            )}
          </div>
        )}
