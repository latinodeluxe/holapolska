...
export default function Leccion98() {
  const [tab, setTab] = useState('repaso')
  const [respuestas, setRespuestas] = useState({})
  const [mostrarResultado, setMostrarResultado] = useState(false)
  const repaso = [
    { categoria: 'Trabajo', frases: [
      { polaco: 'praca / biuro / szef', pronunciacion: 'PRA-tsa / BYU-ro / shef', traduccion: 'trabajo / oficina / jefe' },
      { polaco: 'pensja / urlop / nadgodziny', pronunciacion: 'PEN-sya / UR-lop / nad-go-DZI-ni', traduccion: 'sueldo / vacaciones / horas extras' },
    ]},
  ]
  const examen = [
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        <div className="mb-8">
          <span className="text-sm text-red-500 font-medium">Leccion 98 · Modulo 10</span>
          <h1 className="text-3xl font-bold text-gray-900 mt-1">Repaso trabajo y oficina</h1>
          <p className="text-gray-500 mt-2">Repasa el Modulo 8</p>
        </div>
          ...
          <div>
            {!mostrarResultado ? (
              ...
              <div className="bg-white rounded-2xl p-8 shadow-sm text-center">
                ...
                <a href="/cursos/polaco-a1/leccion-99" className="inline-block mt-6 px-6 py-3 bg-red-500 text-white rounded-xl font-semibold">Siguiente leccion</a>
              </div>
            )}
          </div>
        )}
