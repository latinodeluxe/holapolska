...
export default function Leccion7() {
  ...
  const vocabulario = [
    { polaco: 'jeden', pronunciacion: 'YE-den', traduccion: 'uno', ejemplo: 'Mam jeden brat.' },
    { polaco: 'dwa', pronunciacion: 'dva', traduccion: 'dos', ejemplo: 'Mam dwa koty.' },
    { polaco: 'trzy', pronunciacion: 'tshi', traduccion: 'tres', ejemplo: 'Mam trzy siostry.' },
    { polaco: 'cztery', pronunciacion: 'CHTE-ri', traduccion: 'cuatro', ejemplo: 'Mam cztery lata.' },
    { polaco: 'piec', pronunciacion: 'pyench', traduccion: 'cinco', ejemplo: 'Mam piec ksiazek.' },
    { polaco: 'szesc', pronunciacion: 'sheshch', traduccion: 'seis', ejemplo: 'Mam szesc przyjaciol.' },
    { polaco: 'siedem', pronunciacion: 'SHE-dem', traduccion: 'siete', ejemplo: 'Siedem dni tygodnia.' },
    { polaco: 'osiem', pronunciacion: 'O-shem', traduccion: 'ocho', ejemplo: 'Mam osiem lat.' },
    { polaco: 'dziewiec', pronunciacion: 'DZHE-vyench', traduccion: 'nueve', ejemplo: 'Dziewiec miesiecy.' },
    { polaco: 'dziesiec', pronunciacion: 'DZHE-shench', traduccion: 'diez', ejemplo: 'Mam dziesiec palcow.' },
  ]
  const ejercicios = [
    { pregunta: 'Como se dice "cinco" en polaco?', opciones: ['cztery', 'piec', 'szesc', 'siedem'], correcta: 'piec' },
    { pregunta: 'Que significa "osiem"?', opciones: ['seis', 'siete', 'ocho', 'nueve'], correcta: 'ocho' },
    { pregunta: 'Como se dice "tres" en polaco?', opciones: ['jeden', 'dwa', 'trzy', 'cztery'], correcta: 'trzy' },
    { pregunta: 'Que significa "dziesiec"?', opciones: ['ocho', 'nueve', 'diez', 'siete'], correcta: 'diez' },
    { pregunta: 'Como se dice "seis" en polaco?', opciones: ['piec', 'szesc', 'siedem', 'osiem'], correcta: 'szesc' },
  ]
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        <div className="mb-8">
          <span className="text-sm text-red-500 font-medium">Leccion 7 · Modulo 1</span>
          <h1 className="text-3xl font-bold text-gray-900 mt-1">Numeros del 1 al 10</h1>
          <p className="text-gray-500 mt-2">Aprende a contar en polaco</p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {vocabulario.map((item, idx) => (
              <div key={idx} className="bg-white rounded-2xl p-6 shadow-sm text-center">
                <div className="text-4xl font-bold text-red-500 mb-2">{idx + 1}</div>
                <h3 className="text-xl font-bold text-gray-900">{item.polaco}</h3>
                <p className="text-red-400 text-sm mt-1">{item.pronunciacion}</p>
                <p className="text-gray-600 text-sm mt-1">{item.traduccion}</p>
              </div>
            ))}
          </div>
        )}
        {tab === 'flashcards' && (
          <div className="text-center">
            ...
            <div onClick={() => setVoltear(!voltear)} className="bg-white rounded-2xl shadow-sm p-12 cursor-pointer min-h-64 flex items-center justify-center mb-6">
              {!voltear ? (
                <div><p className="text-6xl font-bold text-red-500 mb-4">{tarjetaActual + 1}</p><p className="text-gray-400 text-sm">Click para ver en polaco</p></div>
              ) : (
                <div><p className="text-4xl font-bold text-gray-900 mb-2">{vocabulario[tarjetaActual].polaco}</p><p className="text-2xl font-bold text-red-500 mb-2">{vocabulario[tarjetaActual].pronunciacion}</p></div>
              )}
            </div>
            ...
            {!mostrarResultado ? (
              ...
              <div className="bg-white rounded-2xl p-8 shadow-sm text-center">
                ...
                <div className="flex gap-4 justify-center mt-8">
                  ...
                  <a href="/cursos/polaco-a1/leccion-8" className="px-6 py-3 bg-red-500 text-white rounded-xl font-semibold">Siguiente leccion</a>
                </div>
              </div>
            )}
          </div>
