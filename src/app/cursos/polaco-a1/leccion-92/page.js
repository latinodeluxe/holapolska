...
export default function Leccion92() {
  const [tab, setTab] = useState('repaso')
  const [respuestas, setRespuestas] = useState({})
  const [mostrarResultado, setMostrarResultado] = useState(false)
  const repaso = [
    { categoria: 'Familia', frases: [
      { polaco: 'mama / tata / brat / siostra', pronunciacion: 'MA-ma / TA-ta / brat / SHYO-stra', traduccion: 'mama / papa / hermano / hermana' },
    ]},
    { categoria: 'Profesiones y colores', frases: [
      { polaco: 'lekarz / nauczyciel', pronunciacion: 'LE-kazh / na-u-CHI-chel', traduccion: 'medico / profesor' },
      { polaco: 'czerwony / niebieski / zielony', pronunciacion: 'cher-VO-ni / nye-BYES-ki / zhe-LO-ni', traduccion: 'rojo / azul / verde' },
    ]},
  ]
  const examen = [
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        <div className="mb-8">
          <span className="text-sm text-red-500 font-medium">Leccion 92 · Modulo 10</span>
          <h1 className="text-3xl font-bold text-gray-900 mt-1">Repaso familia y profesiones</h1>
          <p className="text-gray-500 mt-2">Repasa el Modulo 2</p>
        </div>
          ...
          <div>
            {!mostrarResultado ? (
              ...
              <div className="bg-white rounded-2xl p-8 shadow-sm text-center">
                ...
                <a href="/cursos/polaco-a1/leccion-93" className="inline-block mt-6 px-6 py-3 bg-red-500 text-white rounded-xl font-semibold">Siguiente leccion</a>
              </div>
            )}
          </div>
        )}
