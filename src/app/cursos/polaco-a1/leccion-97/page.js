...
export default function Leccion97() {
  const [tab, setTab] = useState('repaso')
  const [respuestas, setRespuestas] = useState({})
  const [mostrarResultado, setMostrarResultado] = useState(false)
  const repaso = [
    { categoria: 'Salud', frases: [
      { polaco: 'glowa / reka / noga', pronunciacion: 'GWO-va / REN-ka / NO-ga', traduccion: 'cabeza / mano / pierna' },
      { polaco: 'boli mnie / lekarz / apteka', pronunciacion: 'BO-li mnye / LE-kazh / ap-TE-ka', traduccion: 'me duele / medico / farmacia' },
    ]},
  ]
  const examen = [
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        <div className="mb-8">
          <span className="text-sm text-red-500 font-medium">Leccion 97 · Modulo 10</span>
          <h1 className="text-3xl font-bold text-gray-900 mt-1">Repaso salud y cuerpo</h1>
          <p className="text-gray-500 mt-2">Repasa el Modulo 7</p>
        </div>
          ...
          <div>
            {!mostrarResultado ? (
              ...
              <div className="bg-white rounded-2xl p-8 shadow-sm text-center">
                ...
                <a href="/cursos/polaco-a1/leccion-98" className="inline-block mt-6 px-6 py-3 bg-red-500 text-white rounded-xl font-semibold">Siguiente leccion</a>
              </div>
            )}
          </div>
        )}
