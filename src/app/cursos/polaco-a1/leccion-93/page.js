...
export default function Leccion93() {
  const [tab, setTab] = useState('repaso')
  const [respuestas, setRespuestas] = useState({})
  const [mostrarResultado, setMostrarResultado] = useState(false)
  const repaso = [
    { categoria: 'Casa', frases: [
      { polaco: 'kuchnia / lazienka / sypialnia', pronunciacion: 'KUCH-nya / wa-ZHEN-ka / si-PYAL-nya', traduccion: 'cocina / bano / dormitorio' },
      { polaco: 'stol / krzeslo / lozko', pronunciacion: 'stow / KSHEZ-wo / WOZH-ko', traduccion: 'mesa / silla / cama' },
    ]},
  ]
  const examen = [
    { pregunta: 'Como se dice cocina en polaco?', opciones: ['salon', 'kuchnia', 'lazienka', 'sypialnia'], correcta: 'kuchnia' },
    { pregunta: 'Que significa stol?', opciones: ['silla', 'cama', 'mesa', 'sofa'], correcta: 'mesa' },
    { pregunta: 'Como se dice debajo de en polaco?', opciones: ['nad', 'obok', 'pod', 'za'], correcta: 'pod' },
    { pregunta: 'Que significa duzy?', opciones: ['pequeno', 'viejo', 'limpio', 'grande'], correcta: 'grande' },
  ]
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        <div className="mb-8">
          <span className="text-sm text-red-500 font-medium">Leccion 93 · Modulo 10</span>
          <h1 className="text-3xl font-bold text-gray-900 mt-1">Repaso casa y hogar</h1>
          <p className="text-gray-500 mt-2">Repasa el Modulo 3</p>
        </div>
          ...
          <div>
            {!mostrarResultado ? (
              ...
              <div className="bg-white rounded-2xl p-8 shadow-sm text-center">
                ...
                <a href="/cursos/polaco-a1/leccion-94" className="inline-block mt-6 px-6 py-3 bg-red-500 text-white rounded-xl font-semibold">Siguiente leccion</a>
              </div>
            )}
          </div>
        )}
